...
public class InboundMgmtWorker extends Thread {
	...
	@Override
	public void run() {
		logger.info("inbound management worker queue");
		while (true) {
			logger.info("inbound management worker queue in while=="+ManagementQueue.inbound.size());
			if (!forever && ManagementQueue.inbound.size() == 0)
			break;
			
			
			try {
				logger.info("inbound management worker queue in tryyy");
				// block until a message is enqueued
				ManagementQueueEntry msg = ManagementQueue.inbound.take();
				logger.info("inbound management worker queue msggg=="+msg);
				if (logger.isDebugEnabled())
					logger.debug("Inbound management message received");

				Management req = (Management) msg.req;
				logger.info("inbound management worker queue req=="+req);
				if (req.hasBeat()) {
					/**
