...
public class HeartbeatManager extends Thread {
	...
	public void processRequest(Heartbeat req) {
		logger.info("n heartbeat manager process request");
		if (req == null)
			return;
	...
	public void addOutgoingChannel(String nodeId, String host, int mgmtport, Channel ch, SocketAddress sa) {
		if (!outgoingHB.containsKey(ch)) {
			logger.info("in ******** addOutgoingChannel");
			...
			ch.closeFuture().addListener(new CloseHeartListener(heart)); //Anita commented
		} else {
	...
	@Override
	public void run() {
		logger.info("starting HB manager Anittaaaa");

		while (forever) {
			try {
				Thread.sleep(sHeartRate);
				logger.info("outgoingHB.size() in HB===="+outgoingHB.size());
				...
				if (outgoingHB.size() > 0) {
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						try {
							logger.info("sending heartbeat=="+msg);
