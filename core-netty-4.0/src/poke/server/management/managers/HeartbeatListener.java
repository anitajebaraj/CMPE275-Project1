...
public class HeartbeatListener implements MonitorListener {
	...
	public void onMessage(eye.Comm.Management msg) {
		...
		} else if(msg.hasElection() && msg.getElection().getNodeId().equals(data.getNodeId()))
		{
			logger.info("Received election message==="+msg.getElection().getNodeId());
			logger.info("Vote message==="+msg.getElection());
		}
		else
			logger.error("Received Message from wrong channel or unknown host: " + msg.getBeatOrBuilder().getNodeId());
	}
