	...
	public boolean startElection() {
		...
		try {
			Channel ch = connect();
				...
				logger.info("sending election join message");
		

			LeaderElection.Builder le = LeaderElection.newBuilder();
			le.setNodeId("mgmt-" + whoami + "." + N);
			le.setBallotId("fgkf");
			le.setDesc("Voting message");
			le.setVote(VoteAction.ELECTION);
			le.setExpires(System.currentTimeMillis() + 2000);
			logger.info("leader election build");
			Management.Builder m = Management.newBuilder();
			//m.setGraph(n.build());
			m.setElection(le.build());
			
			logger.info("$$$$$$$management build"+channel.channel());
			
			channel.channel().writeAndFlush(m.build());
			
			rtn = true;
			logger.info("election message sent");
